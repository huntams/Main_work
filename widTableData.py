...
import sys
import testcsvread
from PyQt5.QtWidgets import QWidget, QLabel, QPushButton, QApplication, QGridLayout, QComboBox, QLineEdit, QFileDialog, QTableWidget, QTableWidgetItem, QCheckBox
...
class Tablica(QWidget):
    def __init__(self):
        ...
        self.table.setHorizontalHeaderLabels(["","Адрес", "E(упр.)","dt imp", "Ph. U(max)", "t on","t off", "t max", "Аппроксимация", "Особенности"])
    ...
    def zapolnenietablici (self):
        ...
        self.table.setRowCount(len(testcsvread.alldata)) 
        for i, izm in enumerate(testcsvread.alldata):
             self.table.setCellWidget(i,0,QCheckBox())
             self.table.cellWidget(i,0).setChecked(izm["activ"])
